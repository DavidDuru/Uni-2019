...
glimpse(df)
df.mlr <- df %>% filter(OverallSurvival, VitalStatus == "Dead" || VitalStatus == "Alive")
...
df.mlr$OverallSurvival <- log(df.mlr$OverallSurvival)
df.mlr <- na.omit(df.mlr)
df.mlr <- df.mlr %>% select(-"PatientID", -"SampleID")
df.mlr2 <- df.mlr

ggplot(df.mlr, aes(x = OverallSurvival)) + geom_histogram()
