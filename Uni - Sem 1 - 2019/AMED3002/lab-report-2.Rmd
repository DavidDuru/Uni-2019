...
require(stringr)
require(ggthemr)
explore <- read_excel("/home/aidan/Uni/Uni - Sem 1 - 2019/AMED3002/clinical_2014-01-16.xlsx")
colnames(explore)
glimpse(explore)
...
We can perform an ANOVA  to check for a difference in mean survival between disease types.
```{r}
df.GBM <- df %>% filter(df$Disease == "GBM")
df.astro <- df %>% filter(df$Disease == "ASTROCYTOMA")
df.astro <- df %>% filter(df$Disease == "ASTROCYTOMA")
